// Segundo o autor, este apéndice é como uma prévia opcional do conteúdo mais aprofundado dos outros livros.
...
// Here's how you can prove there's two separate values involved:
    ...
    var yourName = myName; // Cópia do valor primitivo
...
// Consider:
    ...
    var yourAddress = myAddress; // Cópia da referência
...
// myAddress and yourAddress have copies of the reference to the single shared object, so an update to one is an update to both.
...
// Isso revela a importância de nomear uma função para rastreio e debug.
...
// Compare the anonymous function expression form to:
    ...
    // someName (nome dado) é associado à function expression no momento da compilação.

    //awesomeFunction (identificador ou nome indireto - quando não tem nome) é associado somente no momento do runtime daquela statement(unidade completa de execução, que costuma terminar com ;).
...
// the association with the identifier awesomeFunction still doesn't happen until runtime at the time of that statement. Those two identifiers don't have to match; sometimes it makes sense to have them be different, other times it's better to have them be the same.
...
// The function expression may get an inferred name, but only if it's one of the assignment forms, not in the (more common!) form of being passed as a function call argument (as in the last line of the snippet). [Bah, isso é verdade]
