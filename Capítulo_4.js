...
// "This is how scopes behave in most languages, which is called lexical scope.
...
// Em outras palavras, é uma decisão tomada em tempo de escrita: onde você coloca uma função/escopo no programa determina qual será a estrutura de escopo dessa parte do programa. [Porção traduzida via Google Tradutor]
...
// "The second pillar of the language is the prototypes system. We covered this topic in-depth in Chapter 3 ("Prototypes")"
...
// But I think that focus has obscured the beauty and power of the prototype system: the ability for two objects to simply connect with each other and cooperate dynamically (during function/method execution) through sharing a this context."
    ...
    //  Para clarificar: quando, por exemplo, a propriedade "topic" não existe diretamente no objeto ao qual o 'this' se refere, ela é buscada na cadeia de protótipos até ser encontrada (ou não - nesse caso retornando 'undefined'.)
...
// With the Grain ('grain' aqui é 'veio da madeira' num sentido conotativo/figurado)
...
// Basicamente, o JavaScript tem suas peculiaridades que deveriam ser usadas conforme as peculiaridades dele. Se for pra tentar fazer o JavaScript imitar outras linguagens, não faz muito sentido. Muita gente não se aprofunda na linguagem, então, o que você for aprender aqui pode gerar conflitos para você, então fique atento e pense por você mesmo, ao invés de só papagaiar o conteúdo desse livro. Contudo, a tendência é que você gere código melhor justamente por saber mais profundamente como o JavaScript funciona. Então, muitas coisas que parecem defeito da linguagem são mais uma espécie reflexo de uma incompreensão de como ela funciona realmente. Foi mais ou menos isso que entendi de forma resumida, não só do Capítulo 4, mas do que o autor vem falando até aqui.
...
// Basicamente, leia na ordem normal 1,2,3,4,5,6. O 5 é muito importante, mas se você não tiver uma boa base dos pilares do JS (que estão no 2, 3 e 4), você pode sofrer um pouco. Quanto mais você se aprofundar no JS, mais você vai gostar do livro 5. E pratique! Pratique, pratique, pratique!
